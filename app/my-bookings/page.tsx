...
export default function SimReservations(){
  ...
  return <PlayerPage title="Sim Reservations">
    ...
    {cancelBooking&&<div className="booking-confirmation-overlay" role="presentation" onClick={()=>!cancelBusy&&setCancelBooking(null)}><div className="booking-confirmation-modal cancel-hours-modal" role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()}><button type="button" className="booking-confirmation-close" disabled={cancelBusy} onClick={()=>setCancelBooking(null)} aria-label="Close cancellation options">×</button><h2>Cancel Reservation Time</h2><p className="muted">Choose the hour or hours you no longer need. The selected time will immediately become available to other players.</p><div className="cancel-hour-options">{(()=>{const start=easternParts(cancelBooking.start_at).minutes/60,end=easternParts(cancelBooking.end_at).minutes/60;const all=Array.from({length:Math.max(0,end-start)},(_,i)=>start+i);return <><label className="cancel-hour-option cancel-all-option"><input type="checkbox" checked={cancelHours.length===all.length} onChange={()=>setCancelHours(cancelHours.length===all.length?[]:all)}/><span><strong>Cancel All</strong><small>{timeText(cancelBooking.start_at)}–{timeText(cancelBooking.end_at)}</small></span></label>{all.map(h=><label className="cancel-hour-option" key={h}><input type="checkbox" checked={cancelHours.includes(h)} onChange={()=>toggleCancelHour(h)}/><span><strong>{hourLabel(h)}–{hourLabel(h+1)}</strong></span></label>)}</>})()}</div><div className="booking-confirmation-actions"><button type="button" className="btn danger" disabled={cancelBusy||cancelHours.length===0} onClick={confirmCancellation}>{cancelBusy?'Cancelling…':'Cancel Selected Time'}</button><button type="button" className="btn secondary" disabled={cancelBusy} onClick={()=>setCancelBooking(null)}>Keep Reservation</button></div></div></div>}
