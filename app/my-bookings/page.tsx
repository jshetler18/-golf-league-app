...
export default function MyBookings(){
 ...
 return <><section className="hero"><div className="eyebrow">Your Reservations</div><h1>My Bookings</h1><p>See and cancel your upcoming simulator times.</p></section>{msg&&<p className="message">{msg}</p>}{items.length===0?<div className="card"><h2>No upcoming reservations</h2><Link href="/book" className="btn">Book the Sim</Link></div>:<div className="grid">{items.map(b=><div className="card" key={b.id}><h2>{dateText(b.start_at)}</h2><p className="stat-small">{timeText(b.start_at)}–{timeText(b.end_at)}</p><button className="btn danger" onClick={()=>cancel(b.id)}>Cancel Reservation</button></div>)}</div>}</>
}
