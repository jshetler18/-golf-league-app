...
type TeamSlot={id:string;team_id:string;weekday:number;start_time:string;start_date:string;end_date:string}
...
const blockStarts=[6,9,12,15,18]
...
export default function Page(){
 ...
 async function load(){
  ...
  const [{data:t},{data:x},{data:m}]=await Promise.all([
   ...
   supabase.from('team_sim_slots').select('id,team_id,weekday,start_time,start_date,end_date').eq('season_id',s.id),
 ...
 useEffect(()=>{
  if(!season)return
  if(selection===MAKEUP){
   if(!editingMakeupId){setDay(1);setH(12);setStart(practiceStart(season.start_date));setEnd(season.end_date)}
   return
  }
  setEditingMakeupId(null)
  const x=slots.find(s=>s.team_id===selection)
  if(x){setDay(x.weekday);setH(slotHour(x.start_time));setStart(x.start_date);setEnd(x.end_date)}
  else{setDay(1);setH(12);setStart(practiceStart(season.start_date));setEnd(season.end_date)}
 },[selection,slots,season,editingMakeupId])

 async function save(e:FormEvent){
  e.preventDefault(); if(!season)return; setSaving(true); setMsg('')
  if(selection===MAKEUP){
   const {data,error}=await supabase.rpc('set_league_makeup_slot',{p_slot_id:editingMakeupId,p_name:'League Make-Up Time',p_weekday:day,p_start_time:`${pad(h)}:00:00`,p_duration_hours:3,p_start_date:start,p_end_date:end,p_season_id:season.id})
  ...
  const {data,error}=await supabase.rpc('set_team_sim_slot',{p_team_id:selection,p_weekday:day,p_start_time:`${pad(h)}:00:00`,p_duration_hours:3,p_start_date:start,p_end_date:end})
 ...
 function editMakeup(s:MakeupSlot){setSelection(MAKEUP);setEditingMakeupId(s.id);setDay(s.weekday);setH(slotHour(s.start_time));setStart(s.start_date);setEnd(s.end_date);window.scrollTo({top:0,behavior:'smooth'})}
 function newMakeup(){if(!season)return;setSelection(MAKEUP);setEditingMakeupId(null);setDay(1);setH(12);setStart(practiceStart(season.start_date));setEnd(season.end_date);window.scrollTo({top:0,behavior:'smooth'})}
 ...
 return <AdminFrame title="Team & League Schedules" description="Set recurring 3-hour simulator blocks for teams and League Make-Up Time in one place.">
  ...
  <div className="card">
   ...
   <form onSubmit={save} className="form-grid">
    ...
    <label className="field">3-Hour Time Block<select value={h} onChange={e=>setH(+e.target.value)}>{blockStarts.map(x=><option value={x} key={x}>{hour(x)}–{hour(x+3)}</option>)}</select></label>
   ...
   <p className="muted">All league schedules use one of five standard 3-hour blocks: 6–9 AM, 9 AM–12 PM, 12–3 PM, 3–6 PM, or 6–9 PM. The 6–9 AM block still reserves the simulator from 6 AM; on the player reservation page the visible 7 AM and 8 AM times will show as unavailable for that league block. Schedules may begin in October for practice. Changing a schedule rebuilds future bookings only; past bookings stay unchanged, and conflicts are reported rather than overwritten.</p>
  </div>
  <section>
  ...
  <section>
   ...
   <div className="card team-sim-team-list-v1241">{makeups.length===0?<p className="muted">No recurring League Make-Up Time is set up yet.</p>:makeups.map(s=><div className="team-sim-team-row-v1241 configured" key={s.id}><div className="team-sim-status-dot-v1241">✓</div><div className="team-sim-team-copy-v1241"><strong>League Make-Up Time</strong><span>{days[s.weekday]}s • {hour(slotHour(s.start_time))}–{hour(slotHour(s.start_time)+3)} • {dateFmt(s.start_date)}–{dateFmt(s.end_date)}</span></div><span className="team-sim-status-pill-v1241 configured">Set Up</span><div className="actions"><button className="btn secondary small" onClick={()=>editMakeup(s)}>Edit</button><button className="btn danger small" onClick={()=>clearMakeup(s)}>Remove</button></div></div>)}</div>
  </section>
 </AdminFrame>
}
