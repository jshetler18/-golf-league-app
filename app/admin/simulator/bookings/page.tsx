 ...
 return <AdminFrame title="Bookings & Block Time" description="View upcoming simulator use and create one-time or recurring simulator blocks.">
  ...
  <div className="card"><h2>{editId?'Edit Recurring Simulator Block':'Add Recurring Simulator Block'}</h2>
   <form onSubmit={saveRecurring} className="form-grid">
    ...
    <label className="field">Starting Time<select value={rHour} onChange={e=>setRHour(+e.target.value)}>{Array.from({length:14},(_,i)=>i+7).map(x=><option value={x} key={x}>{ht(x)}</option>)}</select></label>
    <label className="field">Hours Reserved<select value={rDur} onChange={e=>setRDur(+e.target.value)}>{Array.from({length:14},(_,i)=>i+1).map(x=><option value={x} key={x}>{x} hour{x===1?'':'s'}</option>)}</select></label>
  ...
  <div className="card"><h2>Add One-Time Simulator Block</h2><form onSubmit={add} className="form-grid"><label className="field">Reason<input value={title} onChange={e=>setTitle(e.target.value)}/></label><label className="field">Date<input required type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><label className="field">Start Time<select value={h} onChange={e=>setH(+e.target.value)}>{Array.from({length:14},(_,i)=>i+7).map(x=><option value={x} key={x}>{ht(x)}</option>)}</select></label><label className="field">Length<select value={dur} onChange={e=>setDur(+e.target.value)}>{Array.from({length:14},(_,i)=>i+1).map(x=><option value={x} key={x}>{x} hour{x===1?'':'s'}</option>)}</select></label><button className="btn">Block Simulator Time</button></form></div>
