...
export default function SimulatorAdmin(){
 ...
 const cards=[['/admin/simulator/team-schedules','⛳','Team Schedules',`${counts.scheduled} of ${counts.teams} teams set up`,`${Math.max(0,counts.teams-counts.scheduled)} not set up`],['/admin/simulator/makeup-blocks','🔁','League Make-Up Blocks',`${counts.makeups} recurring block${counts.makeups===1?'':'s'}`,'Reserve backup league time'],['/admin/simulator/bookings','📅','Bookings & Block Time',`${counts.bookings} upcoming item${counts.bookings===1?'':'s'}`,'Add, remove, or block simulator time']]
