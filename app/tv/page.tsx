...
export default function TVLeaderboard(){
 ...
 const rows=useMemo(()=>calc(latestWeek).map((base,i)=>{const t=teams.find(x=>x.id===base.id)!;const wk=(n:number)=>(()=>{const v=scores.find(s=>s.team_id===t.id&&s.week_number===n)?.official_total;return v==null?v:Math.min(Number(v),scoreCap)})();const h=handicaps.find(x=>x.team_id===t.id)?.handicap_points;const old=prevRanks.get(t.id);return{t,rank:i+1,h:h==null?null:Number(h),w1:wk(1),w2:wk(2),w3:wk(3),total:base.total,change:old?old-(i+1):0}}),[teams,scores,handicaps,latestWeek,prevRanks,scoreCap])
 ...
 const w4=(id:string)=>(()=>{const v=scores.find(s=>s.team_id===id&&s.week_number===4)?.official_total;return v==null?v:Math.min(Number(v),scoreCap)})()
