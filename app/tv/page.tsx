...
export default function TVLeaderboard(){
 ...
 useEffect(()=>{
  (async()=>{
   ...
   setScoreCap(
    Number(s.standings_score_cap||30)
   )
 ...
 useEffect(()=>{
  ...
  const load=async()=>{
   ...
   const [
    ...
    supabase
     .from('weekly_scores')
     .select(
      'team_id,week_number,official_total,status'
     )
     .eq(
      'league_month_id',
      monthId
  ...
  /*
   * Backup refresh for the TV
   * in case Wi-Fi or realtime
   * briefly disconnects.
   */
  const refreshTimer=
   window.setInterval(
 ...
 /*
  * Determine which of Weeks
  * 1-3 is currently active.
  */
  ...
  * IMPORTANT:
  *
  * Movement indicators do not
  * activate until EVERY active
  * team has an approved score
  ...
  useMemo(()=>{
   ...
   return addRanks(
    ...
    base=>{
     ...
     /*
      * Movement only activates
      * after the entire week's
      * scores are approved.
      */
     const change=
      latestWeek>1 &&
 ...
 if(showWeek4)
  return(
   ...
   <main
    className=
     "tv-approved tv-week4"
   >

    <header
     className=
      "tv-approved-header tv-week4-header"
    >
 ...
 return(
  ...
  >
   ...
   >
    ...
    <div
     className=
      "tv-approved-row tv-approved-head"
    >
     ...
     .map(
      r=>(
       ...
       >
         ...
         * TOTAL COLUMN
         *
         * Total and movement have
         * their own aligned areas.
         *
         * Movement size now matches
         * the Cup Standings page.
         */}
        <span
         className=
          "tv-approved-total"
