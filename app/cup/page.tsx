 ...
 return <PlayerPage title="Cup Standings"><div className="cup-page-v1231">
  ...
  {!season?<div className="card">No active season found.</div>:<>
   ...
   <section id="match-play" className="cup-match-section-v1231"><div className="section-title compact"><div><h2>Match Play</h2><p className="muted">Week 4 head-to-head results and Cup points by month.</p></div></div>
    {months.map(m=>{const ms=matchups.filter(x=>x.league_month_id===m.id);const champ=champions.find(c=>c.league_month_id===m.id);if(!ms.length&&!champ)return null;return <div className="card cup-month-card-v1231" key={m.id}><div className="cup-month-title-v1231"><h3>{monthName(m)}</h3>{champ&&<span className="pill">🏆 {name(champ.team_id)} — Monthly Champion</span>}</div>{ms.length>0&&<div className="cup-match-list-v1231">{ms.map(x=>{const highWin=x.winner_team_id===x.team_high_id,lowWin=x.winner_team_id===x.team_low_id;return <div className="cup-match-row-v1231" key={`${m.id}-${x.seed_high}-${x.seed_low}`}><div className="cup-seeds-v1231">{x.seed_high} vs {x.seed_low}</div><div className={`cup-match-team-v1231 ${highWin?'winner':''}`}><strong>{name(x.team_high_id)}</strong>{x.high_points_awarded!==null&&<span>{x.high_points_awarded.toLocaleString()} pts</span>}</div><div className="cup-match-vs-v1231">vs</div><div className={`cup-match-team-v1231 ${lowWin?'winner':''}`}><strong>{name(x.team_low_id)}</strong>{x.low_points_awarded!==null&&<span>{x.low_points_awarded.toLocaleString()} pts</span>}</div><div className="cup-match-result-v1231">{x.winner_team_id?<><small>Winner</small><strong>{name(x.winner_team_id)}</strong></>:<span>Pending</span>}</div></div>})}</div>}</div>})}
