...
export default function ScorecardOfficialPage(){
 ...
 const load=useCallback(async()=>{
  ...
  if(!user){setAllowed(false);setLoadingRows(false);return}
  ...
  setAllowed(can)
  if(!can){setLoadingRows(false);return}
  const {data:{session}}=await supabase.auth.getSession()
  if(!session?.access_token){setLoadingRows(false);return}
  const r=await fetch('/api/scorecard-review',{headers:{Authorization:`Bearer ${session.access_token}`},cache:'no-store'})
  const j=await r.json()
  if(!r.ok){setMsg(j.error||'Unable to load scorecards.');setLoadingRows(false);return}
  setRows(j.rows||[])
