...
export default function Home(){
 const [profile,setProfile]=useState<any>(null),[open,setOpen]=useState(false),[unread,setUnread]=useState(0),[chatUnread,setChatUnread]=useState(0),[youtubeLive,setYoutubeLive]=useState(false); const wrap=useRef<HTMLDivElement>(null)
 ...
 useEffect(()=>{let alive=true;const check=async()=>{try{const r=await fetch('/api/youtube/live',{cache:'no-store'});const j=await r.json();if(alive)setYoutubeLive(!!j.isLive)}catch{}};check();const timer=window.setInterval(check,60000);return()=>{alive=false;window.clearInterval(timer)}},[])
