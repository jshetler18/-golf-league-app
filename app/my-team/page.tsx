  ...
  return <PlayerPage title="">
    ...
    <div className="my-team-grid">
      ...
      <div className="card">
        ...
        {!monthInfo?.matchup?<p className="muted">This team's Week 4 matchup has not been set yet.</p>:<div className="cup-match-card-v1256 my-team-cup-match-v1296"><div className="cup-seed-banner-v1256">#{monthInfo.matchup.seed_high} Seed <span>HEAD TO HEAD</span> #{monthInfo.matchup.seed_low} Seed</div><div className="cup-head-to-head-v1256"><div className={`cup-fighter-v1256 ${monthInfo.matchup.winner_team_id===team.id?'winner':''}`}><strong>{team.name}</strong><b>{monthInfo.teamPoints==null?'—':monthInfo.teamPoints.toLocaleString()}</b><small>Cup Points</small>{monthInfo.matchup.winner_team_id===team.id&&<em>🏆 WINNER</em>}</div><div className="cup-vs-badge-v1256">VS</div><div className={`cup-fighter-v1256 ${monthInfo.matchup.winner_team_id&&monthInfo.matchup.winner_team_id!==team.id?'winner':''}`}><strong>{monthInfo.opponent||'Opponent'}</strong><b>{monthInfo.opponentPoints==null?'—':monthInfo.opponentPoints.toLocaleString()}</b><small>Cup Points</small>{monthInfo.matchup.winner_team_id&&monthInfo.matchup.winner_team_id!==team.id&&<em>🏆 WINNER</em>}</div></div>{!monthInfo.matchup.winner_team_id&&<div className="cup-pending-v1256">Matchup Pending</div>}</div>}
    ...
    <div className="card">
      ...
      <div className="my-team-roster">{roster.filter(p=>p.team_id===team.id).sort((a,b)=>(a.id===team.captain_player_id?-1:b.id===team.captain_player_id?1:a.full_name.localeCompare(b.full_name))).map(p=><div key={p.id} className="my-team-player"><span className="my-team-avatar">{playerAvatars[p.id]?<img src={playerAvatars[p.id]} alt={`${p.full_name} profile`}/>:<span aria-hidden="true">👤</span>}</span><div><strong>{p.full_name}{p.id===team.captain_player_id&&<span className="captain-mark-v1357"> (C)</span>}</strong><small className="my-team-tee">{p.official_tee_color&&<span className={`tee-square tee-${p.official_tee_color.toLowerCase()}`} aria-hidden="true"></span>}<span>{p.official_tee_color?(teeLabels[p.official_tee_color.toLowerCase()]||`${p.official_tee_color} Tees`):'Tee not set'}</span></small></div></div>)}</div>
    </div>
  </PlayerPage>
}
