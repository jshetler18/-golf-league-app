...
async function syncOpenMeetingRoster(db:any,meeting:{id:string,season_id:string,is_open:boolean}){
 ...
 const [{data:players,error:pErr},{data:teams,error:tErr},{data:existing}]=await Promise.all([
  ...
  db.from('teams').select('id,name').eq('season_id',meeting.season_id).eq('is_active',true),
  db.from('league_meeting_invitees').select('player_id').eq('meeting_id',meeting.id)
 ]);
 if(pErr||tErr)return;
...
export async function GET(req:NextRequest){const token=req.nextUrl.searchParams.get('token')||'';if(!token)return NextResponse.json({error:'RSVP link is invalid.'},{status:400});const db=admin();const {data:m,error}=await db.from('league_meetings').select('id,title,meeting_at,location,message,is_open,season_id').eq('public_token',token).maybeSingle();if(error||!m)return NextResponse.json({error:'This RSVP link was not found.'},{status:404});await syncOpenMeetingRoster(db,m);const {data:people,error:pErr}=await db.from('league_meeting_invitees').select('id,player_name,team_name,is_coming,response_status,responded_at').eq('meeting_id',m.id).order('team_name').order('player_name');if(pErr)return NextResponse.json({error:pErr.message},{status:400});return NextResponse.json({meeting:m,people:people||[]})}
