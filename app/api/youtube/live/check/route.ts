...
export async function POST(req:NextRequest){
  try{
    ...
    if(!status.configured)return NextResponse.json({ok:true,configured:false,isLive:false,sent:0})
    if(!status.isLive||!status.videoId)return NextResponse.json({ok:true,configured:true,isLive:false,sent:0})

    const url=process.env.NEXT_PUBLIC_SUPABASE_URL
    const adminKey=process.env.SUPABASE_SECRET_KEY
    const privateKey=process.env.VAPID_PRIVATE_KEY
    if(!url||!adminKey||!privateKey)return NextResponse.json({error:'Live push notifications are not fully configured.'},{status:503})
    ...
    if(existing?.notified_at)return NextResponse.json({ok:true,isLive:true,alreadyNotified:true,sent:0,videoId:status.videoId})
    ...
    return NextResponse.json({ok:true,isLive:true,videoId:status.videoId,sent,failed})
  }catch(error:any){
