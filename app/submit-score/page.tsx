...
export default function SubmitScore(){
 ...
 return <PlayerPage title="Submit Score"><div className="simple-mobile-page submit-score-page">
   ...
   {myPendingRounds.length>0&&<section className="pending-alerts-v1313" aria-label="Scorecards awaiting approval"><div className="pending-alerts-heading-v1313"><strong>Awaiting Approval</strong><span>{myPendingRounds.length===1?'You have 1 scorecard waiting for review.':`You have ${myPendingRounds.length} scorecards waiting for review.`}</span></div>{myPendingRounds.map(r=>{const m=ctx.months.find(x=>x.id===r.league_month_id);const teamName=r.teams?.name||ctx.teams.find(t=>t.id===r.team_id)?.name||'Team';return <div className="pending-alert-card-v1313" key={r.id}><strong>{teamName} · {m?monthName(m):'League'} · Week {r.week_number}</strong><span>Your {m?monthName(m):'league'} Week {r.week_number} scorecard and submitted score are waiting for a Scorecard Official to review.</span></div>})}</section>}
   <div className="card">
     ...
     {!changeRound&&(existing?.status==='pending'?<div className="pending-rounds-wrap-v1304"><button type="button" className="btn secondary submit-another-round-v1304" onClick={()=>{setMsg('');setRoundMonthChoice('');setRoundWeekChoice('');setChangeRound(true)}}>Submit Another Round</button></div>
