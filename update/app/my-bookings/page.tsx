...
export default function SimReservations(){
  ...
  function isPastSlot(hour:number){if(date!==today)return false;const parts=new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).formatToParts(new Date());const get=(t:string)=>Number(parts.find(p=>p.type===t)?.value||0);return hour*60<=(get('hour')*60+get('minute'))}
  ...
  async function cancel(id:string){if(!confirm('Cancel this simulator reservation? The time will immediately become available to others.'))return;const {error}=await supabase.from('bookings').update({status:'cancelled'}).eq('id',id);setMessage(error?error.message:'Reservation cancelled.');if(!error){try{const {data:{session}}=await supabase.auth.getSession();if(session?.access_token)await fetch('/api/push/booking-admin',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({bookingId:id,action:'cancelled'})})}catch{}await load(true)}}
