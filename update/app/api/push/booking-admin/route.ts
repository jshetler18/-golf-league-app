...
export async function POST(req:NextRequest){
 try{
  ...
  const {bookingId,action='booked'}=await req.json()
  ...
  if(b.kind!=='personal'||b.user_id!==user.id||(!isCancellation&&b.status!=='active')||(isCancellation&&b.status!=='cancelled'))return NextResponse.json({error:'Reservation cannot be announced.'},{status:403})
  const {data:booker}=await admin.from('profiles').select('full_name,email').eq('id',user.id).maybeSingle()
  const {data:recipients}=await admin.from('profiles').select('id,role,is_account_approver').eq('status','approved')
  const ids=(recipients||[]).filter((x:any)=>x.role==='admin'||x.is_account_approver===true).map((x:any)=>x.id)
  ...
  const body=isCancellation?`${name} cancelled ${fmtDate(b.start_at)} from ${fmtTime(b.start_at)} to ${fmtTime(b.end_at)}.`:`${name} booked ${fmtDate(b.start_at)} from ${fmtTime(b.start_at)} to ${fmtTime(b.end_at)}.`
  const title=isCancellation?'Golf Sim Reservation Cancelled':'Golf Sim Reservation Confirmation'
  let sent=0
  for(const s of subs||[]){if(!s.p256dh||!s.auth)continue;try{await webpush.sendNotification({endpoint:s.endpoint,keys:{p256dh:s.p256dh,auth:s.auth}},JSON.stringify({title,body,url:'/admin/simulator/bookings',tag:`admin-booking-${isCancellation?'cancelled-':''}${b.id}`,kind:isCancellation?'admin-booking-cancelled':'admin-booking'}));sent++}catch(e:any){if(e?.statusCode===404||e?.statusCode===410)await admin.from('push_subscriptions').delete().eq('id',s.id)}}
